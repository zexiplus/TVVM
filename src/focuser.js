...
class Focuser {
  ...
  init(vm, options) {
    ...
    if (this.focusOptions.defaultFocusIndex) {
      let IndexArr =
        options.focus && options.focus.defaultFocusIndex.split("-");
    ...
    this.focusState = {
      currentIndexString:
        (options.focus && options.focus.defaultFocusIndex) || "",
      currentRowIndex,
      currentColIndex
    };
        ...
        Object.keys(this.keysMap).forEach(key => {
          ...
          if (defaultFocusOptions.keysMap[key]) {
            this.keysMap[key].codes = options.focus.keysMap[key].codes
              ? [
                  ...new Set(
                    defaultFocusOptions.keysMap[key].codes.concat(
                      options.focus.keysMap[key].codes
                    )
                  )
                ]
              : this.keysMap[key].codes;
          } else {
            this.keysMap[key].codes = options.focus.keysMap[key].codes;
          }
        });
  ...
  addFocusMap(key, node) {
    ...
    keys.forEach(item => {
      if (item in this.focusElementMap) {
        return console.warn(
          `t-focus should be unique in one TVVM page but t-focus=${item} has already exist`
        );
      }
  ...
  isTopBoundary() {
    ...
    while (
      this.focusElementMap[indexString] &&
      this.focusElementMap[this.focusState.currentIndexString] ===
        this.focusElementMap[indexString]
    ) {
      rowIndex--;
  ...
  isRightBoundary() {
    ...
    let colIndex = this.focusState.currentColIndex;
    if (
      colIndex === this.indexMap[rowIndex][this.indexMap[rowIndex].length - 1]
    ) {
      return true;
    }
    colIndex++;
  ...
  moveUp(event, node, index) {
    this.keysMap["up"].handler &&
      this.keysMap["up"].handler(event, node, index);
  ...
  moveDown(event, node, index) {
    this.keysMap["down"].handler &&
      this.keysMap["down"].handler(event, node, index);
  ...
  moveLeft(event, node, index) {
    this.keysMap["left"].handler &&
      this.keysMap["left"].handler(event, node, index);
    if (this.isLeftBoundary()) {
      if (this.focusOptions.circle.horizontal) {
        this.removeFocus(index);
        let rowIndex = this.focusState.currentRowIndex;
        let colIndex = this.indexMap[rowIndex][
          this.indexMap[rowIndex].length - 1
        ];
        let indexString = [rowIndex, colIndex].join("-");
        this.setFocus(indexString);
      }
  ...
  moveRight(event, node, index) {
    this.keysMap["right"].handler &&
      this.keysMap["right"].handler(event, node, index);
  ...
  move(direction, event) {
    ...
    runner[direction].handler.call(
      this,
      event,
      this.focusState.currentFocusElement,
      this.focusState.currentIndexString
    );
  }
}
